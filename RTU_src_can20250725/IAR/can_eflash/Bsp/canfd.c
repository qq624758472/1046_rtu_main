...
extern uint32_t Canfd1rx_bytes[16];
extern uint32_t Canfd0rx_bytes[16];

extern uint8_t Canfd3rx_flags;
extern uint32_t Canfd3rx_bytes;


extern uint8_t Receive_data1[16][64]; // 最大接受16个64字节数据
extern uint8_t Receive_data0[16][64];
...
void CANFD1_IRQ_Handler()
{
    if (CANFD_GetITStatus(CANFD1, CANFD_FLAG_RXOK) != RESET)
    {
        ...
        //Canfd1rx_flags = 1;
        can1Info[can1Ptr] = CANFD_GetReceiveInfo(CANFD1);
        Canfd1rx_bytes[can1Ptr] = CANFD_GetReceiveData(CANFD1, Receive_data1[can1Ptr]);
        /* Clear the interrupt pending bits */
        can1Ptr++;
        CANFD_ClearITPendingBit(CANFD1, CANFD_CLEAR_RXOK);
        
        //Printf("Canfd1rx_bytes=%d\r\n", Canfd1rx_bytes[can1Ptr]);
    }
}
void CANFD0_IRQ_Handler()
{
    if (CANFD_GetITStatus(CANFD0, CANFD_FLAG_RXOK) != RESET)
    {
        ...
        //Canfd0rx_flags = 1;
        can0Info[can0Ptr] = CANFD_GetReceiveInfo(CANFD0);
        Canfd0rx_bytes[can0Ptr] = CANFD_GetReceiveData(CANFD0, Receive_data0[can0Ptr]);
        /* Clear the interrupt pending bits */
        can0Ptr++;
        CANFD_ClearITPendingBit(CANFD0, CANFD_CLEAR_RXOK);
        
        //Printf("Canfd0rx_bytes=%d\r\n", Canfd0rx_bytes[can0Ptr]);
    }
}
