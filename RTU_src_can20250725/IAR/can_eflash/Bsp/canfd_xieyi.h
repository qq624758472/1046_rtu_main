...
typedef enum
{
    ...
    CMD_INTERSTELLAR_RETURN = 0x08,  //请求星间模块返回数据
    CMD_INTERSTELLAR_RESET = 0x09,   // 星间模块复位
    CMD_TRANSPARENT_DATA = 0x0a,     // 透传模式数据发送到星间模块
    CMD_DIANJI_CTL = 0x0b,           // 电机控制指令转发
    CMD_XINGJIAN_HEX_CMD = 0x0c,     // 星间模块的寄存器控制指令转发
    CMD_INTERSTELLAR_CONFIG = 0x0d,  // 请求星间模块返回数据
...
typedef struct
{
    ...
    uint8_t large_electric_ctr;           // 大电流配电状态
    uint16_t small_electric_ctr;          // 小电流配电状态
    uint16_t out_adc_data[16];            // 16路对外采集温度 或者电压模拟量
    uint16_t small_power_peidian_adc[32]; // 16路小功率配电模拟量 --就是小电流
    uint16_t large_power_peidian_adc[4];  // 2路大功率配电模拟量  --就是大电流
...
// 协议解析
extern uint8_t can_xieyi_test(uint8_t *cmd, uint32_t data_length);
